...
class JacobianDemo():
    ...
    def ellipse(t,f=0.5,ry=.15,rz=.10):
        ...
        Rdes = np.eye(3)  # No rotation for the ellipse
        ang_vdes = np.zeros(3)  # No angular velocity for the ellipse
        ## END STUDENT CODE
    ...
    def line(t,f=0.1,L=.15):
        ...
        #xdes = JacobianDemo.x0
        #vdes = np.array([0,0,0])
        xdes = x0 + np.array([L*sin(2*pi*f*t),0,0])
        vdes = np.array([L*2*pi*f*cos(2*pi*f*t),0,0])
